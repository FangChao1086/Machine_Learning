...
* 训练
  ...
  * 得到的特征输入到SVM进行分类，查看feature vector对应的region proposal是具体类还是背景。进而得到具体类别bounding box

  ![rcnn——pred](https://i.ibb.co/DbQvNw6/rcnn-pred.jpg)  
  ![rcnn-pre-class-svm](https://i.ibb.co/SmnpYtJ/rcnn-pre-class-svm.jpg)  
  
* 位置精修
  * 训练样本：判定为本类的候选框中与真值重叠面积大于0.6的候选框 
  * 回归器：对每一类目标，使用线性脊回归器进行精修。
