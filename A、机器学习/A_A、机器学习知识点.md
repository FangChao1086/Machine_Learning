...
* [独立与不相关](#独立与不相关)
* [不平衡类别问题](#不平衡类别问题)
* [过拟合与欠拟合的解决办法](#过拟合与欠拟合的解决办法)
* [交叉验证](#交叉验证)
  * [k_fold](#k_fold)
  * [leave_p_out](#leave_p_out)
  * [交叉验证防止过拟合的原因](#交叉验证防止过拟合的原因)
* [几种距离度量公式](#几种距离度量公式)
* [L1和L2](#L1和L2)
* [降维](#降维)
  * [PCA](#PCA)
...
* [评估指标](#评估指标)  
  * [Rss,Tss,Ess,R^2](#r2)
* [熵、基尼系数、条件熵_联合熵_边缘熵、互信息、相对熵](#熵、基尼系数、条件熵_联合熵_边缘熵、互信息、相对熵)
* [迁移学习](#迁移学习)
* [补充](#补充)
...
<span id="特征工程"></span>
## [特征工程](https://tianchi.aliyun.com/mas-notebook/preview/36434/bffd10b0-18d3-11e9-bace-61d7df7cd8c1.ipynb/-1?lang=zh-cn#4.1-%E4%B8%BB%E6%88%90%E5%88%86%E5%88%86%E6%9E%90%E6%B3%95%EF%BC%88PCA%EF%BC%89)
...
<span id="特征编码"></span>
## 特征编码
* Onehot encoding：对于**定类**类型的数据，建议使用one-hot encoding
  * 优点：解决了分类器不好处理分类数据的问题，在一定程度上也起到了扩充特征的作用。它的值只有0和1，不同的类型存储在垂直的空间。
  * 缺点：当类别的数量很多时，特征空间会变得非常大，容易造成维度灾难
  * 适用于：
    * 男、女
    * 各省市  
    ![onehot](https://i.ibb.co/T2FjDFm/onehot.jpg)
* Label encoding：对于**定序**类型的数据，建议使用label encoding
...
<span id="几种距离度量公式"></span>
## 几种距离度量公式
[链接](https://my.oschina.net/hunglish/blog/787596)  
* 欧式距离
...
<span id="PCA"></span>
### PCA:投影后的数据具有最大方差  
[参考链接：主成分分析(PCA)](https://blog.csdn.net/u012162613/article/details/42177327)  
...
* **PCA选择维数的依据：方差百分比**，这里选用0.99
  * 计算公式：  
  ![pca_方差百分比](https://i.ibb.co/frc1K5N/pca.png)
<span id="LDA"></span>
### LDA:投影后的数据具有最大区分度
* 有监督降维
...
**二分类**  
![roc_auc](https://i.ibb.co/WGkYCDf/auc-roc.png)  
...
<span id="r2"></span>
## Rss,Tss,Ess,R^2
![r2](https://i.ibb.co/PZD6CyW/r2.png)  

<span id="熵、基尼系数、条件熵_联合熵_边缘熵、互信息、相对熵"></span>
## 熵、基尼系数、条件熵_联合熵_边缘熵、互信息、相对熵  
[参考链接：熵、基尼系数、条件熵_联合熵_边缘熵、互信息、相对熵](https://blog.csdn.net/l275940071/article/details/80010924)  
* 熵  
...
<span id="迁移学习"></span>
## 迁移学习
* 模型初始化：就是把之前训练好的模型直接拿来用，可以充分利用之前数据信息，而且能够避免自己实验数据量较小等问题
