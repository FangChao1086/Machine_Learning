...
给定样本数量：m; label:{-1,1}  
* 初始化训练样本的权值  
$$D(1)=(\omega_{11},\omega_{12},...,\omega_{1m});  \omega_{1i}=\frac{a}{b};i=1,2,…,m$$  
* 多轮迭代产生T个弱分类器  
对于每一个`t = 1,2,..,T:`
  * 使用`D(t)`的训练集进行训练，得到下一个弱分类器  
  $$G_t(x):\chi\rightarrow{-1,+1}$$  
  * 计算弱分类器`G_t(x)`的分类误差率  
  $$e_t=P(G_t(x_i)\neq{y_i})=∑i=\sum_{i=1}^{m}\omega_{ti}I(G_t(x_i)\neq{y_i})$$  
  * 计算弱分类器在最终分类器中的系数  
  ...
  * 更新训练数据集的权值分布  
  $$D(t+1)=\left (\omega_{t+1,1},\omega_{t+1,2},...,\omega_{t+1,m}\right)$$
    * `for i=1,...,m:`  
      * 更新各个样本的权值系数  
      $$\omega_{m+1,i}=\frac {\omega_{m,i}*exp(-\alpha_m*y_iG_m(x_i)))}{Z_m}$$  
      * `z_t`是规范化因子
      $$Z_t=\sum_{i=1}^{m}\omega_{t,i}exp(−\alpha_tG_t(x_i))$$  
      简化为：  
      $$\omega_{m+1,i} =\{\begin{matrix}\frac{\omega{m,i}}{Z_m}e^{-\alpha_m},G_m(x_i)=y_i\\ 
\frac{\omega{m,i}}{Z_m}e^{\alpha_m},G_m(x_i)\neq_y_i\end{matrix}$$  
$$w_{m+1,i}=\left{\begin{matrix} \frac{w_{m,i}}{Z_m}e^{\color{Red} {-\alpha_m}}, & G_m(x_i)=y_i \ \frac{w_{m,i}}{Z_m}e^{\color{Red} {\alpha_m}},& G_m(x_i)\neq y_i \end{matrix}\right.$$
* 集成T个弱分类器为强分类器  
...
**簇质心k个数的选取**
* 评估指标：sklearn.metrics.calinski_harabaz_score(X,y_pred),越大越好
### k-means与KNN
