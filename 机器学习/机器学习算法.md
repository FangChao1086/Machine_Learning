...
2. 对 m=1,2,..,M
  ...
  * 更新模型 f_m(x)  
  f_m(x)={\color{Red} f_{m-1}(x)+\beta b(x;\gamma)}  
3. 得到加法模型  
...
* 前向分步算法将同时求解m=1,2,..,M所有参数(β_m,γ_m)的问题简化为逐次求解各(β_m,γ_m)的优化问题——思想上有点像梯度下降  
#### 前向分步算法与 AdaBoost
